function openCart()   { var c=document.getElementById('cartPanel'),o=document.getElementById('cartOverlay'); if(c) c.classList.add('open'); if(o) o.classList.add('open'); }
function closeCart()  { var c=document.getElementById('cartPanel'),o=document.getElementById('cartOverlay'); if(c) c.classList.remove('open'); if(o) o.classList.remove('open'); }
function toggleCart() { var c=document.getElementById('cartPanel'); if(c&&c.classList.contains('open')) closeCart(); else openCart(); }

function refreshSplitCart() {
    var container=document.getElementById('splitCartContent');
    if (container) { container.innerHTML=renderCartPanelContent(refreshSplitCart); bindCartEvents(refreshSplitCart); }
}
function refreshGlobalCart() {
    var container=document.getElementById('cartPanelContent');
    if (container) { container.innerHTML=renderCartPanelContent(refreshGlobalCart); bindCartEvents(refreshGlobalCart); }
}

function renderCartPanelContent(refreshFn) {
    var emptyCart='<div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:32px 0"><div style="background:linear-gradient(135deg,#e8eeff,#f0f4ff);border-radius:50%;width:88px;height:88px;display:flex;align-items:center;justify-content:center;font-size:44px">🛒</div><div style="font-size:17px;font-weight:700;color:#9fa8da">السلة فارغة</div><div style="font-size:14px;color:#b0bec5;text-align:center;line-height:1.7">اضغط على منتج<br>أو امسح الباركود 📷</div></div>';
    ...
    return '<div style="display:flex;flex-direction:column;height:100%;gap:8px">'+
        ...
        '<div style="display:flex;align-items:center;gap:8px"><div style="background:#0277bd;border-radius:8px;width:30px;height:30px;display:flex;align-items:center;justify-content:center;font-size:16px">🛒</div><div style="color:#fff;font-weight:800;font-size:16px">السلة</div></div>'+
        (S.cart.length>0?'<div style="display:flex;gap:6px"><span style="background:rgba(255,255,255,.15);color:#fff;border-radius:20px;padding:3px 11px;font-size:13px;font-weight:700">'+S.cart.length+' صنف</span><span style="background:#0277bd;color:#fff;border-radius:20px;padding:3px 11px;font-size:13px;font-weight:700">'+cTotal()+' دج</span></div>':'<span style="color:rgba(255,255,255,.4);font-size:13px">فارغة</span>')+
        '</div>'+
        cartCards+discRow+payRow+'</div>';
}

function bindCartEvents(refreshFn) {
    document.querySelectorAll('[data-rem]').forEach(function(btn){ btn.onclick=function(){ remFromCart(+btn.dataset.rem); refreshFn(); }; });
    document.querySelectorAll('[data-inc]').forEach(function(btn){ btn.onclick=function(){ changeCartQty(+btn.dataset.inc,1); refreshFn(); }; });
    document.querySelectorAll('[data-dec]').forEach(function(btn){ btn.onclick=function(){ changeCartQty(+btn.dataset.dec,-1); refreshFn(); }; });
    document.querySelectorAll('[data-price]').forEach(function(inp){ inp.oninput=function(){ updateCartItemPrice(+inp.dataset.price,+inp.value); refreshFn(); }; });
    document.querySelectorAll('[data-disc]').forEach(function(btn){ btn.onclick=function(){ S.disc=+btn.dataset.disc; refreshFn(); }; });
    var payBtn=document.getElementById('payBtn'); if(payBtn) payBtn.onclick=function(){ saveSale(null); };
    var clrBtn=document.getElementById('clrCartBtn'); if(clrBtn) clrBtn.onclick=function(){ clrCart(); refreshFn(); };
    var printBtn=document.getElementById('printBtn'); if(printBtn) printBtn.onclick=printReceipt;
    document.querySelectorAll('[data-client]').forEach(function(btn){ btn.onclick=function(){ if(confirm("على حساب "+btn.dataset.cname+"؟")) saveSale({id:+btn.dataset.client,name:btn.dataset.cname}); }; });
}
