...
// ✅ الإصلاح: tFlixyProfit و tFT يقرآن من النظام الجديد (hch_flixy_combined) أولاً
function tFlixyProfit() {
    if (typeof flixyOperations !== 'undefined' && typeof idoomOperations !== 'undefined') {
        ...
        for (var i = 0; i < flixyOperations.length; i++) {
            ...
            if (op.type === "sell" && op.date === today) profit += op.profit;
        }
        for (var j = 0; j < idoomOperations.length; j++) {
            var op2 = idoomOperations[j];
            if (op2.type === "sell" && op2.date === today) profit += op2.profit;
        }
        return profit;
    }
    ...
    }).reduce(function(a, f) { return a + (f.profit || 0); }, 0);
}
...
function tFT() {
    if (typeof flixyOperations !== 'undefined' && typeof idoomOperations !== 'undefined') {
        ...
        for (var i = 0; i < flixyOperations.length; i++) {
            var op = flixyOperations[i];
            if (op.type === "sell" && op.date === today) total += op.amount;
        }
        for (var j = 0; j < idoomOperations.length; j++) {
            var op2 = idoomOperations[j];
            if (op2.type === "sell" && op2.date === today) total += op2.total;
        }
        return total;
    }
    return S.flixy.filter(function(f) {
...
function mFlixyProfit() {
    if (typeof flixyOperations !== 'undefined' && typeof idoomOperations !== 'undefined') {
        ...
        for (var i = 0; i < flixyOperations.length; i++) {
            ...
            if (d.getMonth() === nm() && d.getFullYear() === ny()) profit += op.profit;
        }
        for (var j = 0; j < idoomOperations.length; j++) {
            var op2 = idoomOperations[j];
            if (op2.type !== "sell") continue;
            var d2 = new Date(op2.date);
            if (d2.getMonth() === nm() && d2.getFullYear() === ny()) profit += op2.profit;
        }
        return profit;
    }
    return S.flixy.filter(function(f) {
...
function mFT() {
    if (typeof flixyOperations !== 'undefined' && typeof idoomOperations !== 'undefined') {
        ...
        for (var i = 0; i < flixyOperations.length; i++) {
            ...
            if (op.type !== "sell") continue;
            var d = new Date(op.date);
            if (d.getMonth() === nm() && d.getFullYear() === ny()) total += op.amount;
        }
        for (var j = 0; j < idoomOperations.length; j++) {
            var op2 = idoomOperations[j];
            if (op2.type !== "sell") continue;
            var d2 = new Date(op2.date);
            if (d2.getMonth() === nm() && d2.getFullYear() === ny()) total += op2.total;
        }
        return total;
    }
    return S.flixy.filter(function(f) {
...
function mPT() {
    return S.pjobs.filter(function(p) {
        ...
        return d.getMonth() === nm() && d.getFullYear() === ny();
    }).reduce(function(a, p) { return a + p.price; }, 0);
}
function mET() {
