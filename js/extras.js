// ============================================================
//  زر الوضع الليلي — نسخة جديدة كلياً (مُستقل عن render)
// ============================================================
function createDarkModeBtn() {
    // لا تُعيد الإنشاء إن كان موجوداً
    if (document.getElementById('dmBtn')) return;

    var btn = document.createElement('button');
    btn.id = 'dmBtn';

    // القراءة من S.settings فقط
    var isDark = !!(S.settings && S.settings.darkMode);
    applyDark(isDark);
...
// ============================================================
//  liveBar — لوحة الأداء اليومي (ديسكتوب فقط)
// ============================================================
function injectLiveBar() {
    ...
    var barsHTML = last7.map(function(d, idx) {
        ...
        return '<div style="display:flex;flex-direction:column;align-items:center;gap:4px;flex:1">' +
            '<span style="font-size:9px;color:' +
                (d.val > 0 ? (isToday ? 'var(--green)' : 'var(--blue)') : 'var(--text3)') +
                ';font-weight:700">' + (d.val > 0 ? fmt(d.val) : '—') + '</span>' +
            '<div style="width:100%;background:var(--bg2);border-radius:4px;height:52px;' +
                 'display:flex;align-items:flex-end;overflow:hidden">' +
                '<div style="width:100%;height:' + pct + '%;background:linear-gradient(to top,' +
                    (isToday ? '#059669,#10b981' : 'var(--blue),#7eb3ff') +
    ...
    var topHTML = topItems.length === 0
        ...
        : topItems.map(function(it, i) {
            var medals = ['🥇','🥈','🥉'];
            var col = (typeof CC !== 'undefined' && CC[it.cat]) || 'var(--blue)';
            return '<div style="display:flex;align-items:center;gap:8px;padding:6px 0;' +
                        'border-bottom:1px solid var(--border)">' +
                '<span>' + medals[i] + '</span>' +
                '<span style="flex:1;font-size:12px;font-weight:600;color:var(--text);' +
                      'white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + esc(it.n) + '</span>' +
    ...
    bar.style.cssText =
        ...
        'border-radius:20px;padding:18px 20px;display:flex;align-items:flex-start;' +
    ...
    bar.innerHTML =
        '<div style="width:100%;display:flex;align-items:center;gap:10px;margin-bottom:12px;' +
            ...
            '<span style="font-size:11px;font-weight:700;color:var(--green);' +
                  ...
                  'padding:3px 10px;border-radius:99px">🟢 مباشر</span>' +
        ...
        '<div style="flex:1;min-width:150px">' +
            ...
            [
                ...
                return '<div style="background:var(--bg2);border-radius:9px;padding:8px 9px;' +
