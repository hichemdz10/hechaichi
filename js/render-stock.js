...
function renderStock() {
    ...
    // ===== قسم المنتجات الناقصة =====
    var lowStockItems = S.stock.filter(function(s) {
        return s.q <= (S.settings.lowStockThreshold || 5);
    });
    var lowStockSection = '';
            ...
            '<div class="low-stock-section">' +
                ...
                '<div class="low-stock-content">' +
                    '<div class="category-table-wrapper">' +
                        '<table class="stock-table low-stock-table">' +
                            '<thead><tr>' +
                                '<th>🏪 كمية المحل</th>' +
                                '<th>الصنف</th>' +
                                '<th>💰 البيع</th>' +
        ...
        '<div class="stock-form-card">' +
            ...
            '<div class="stock-form-body">' +
                ...
                '<div class="form-grid">' +
                    ...
                    '<select class="inp" onchange="S.form.cat=this.value">' +
                        CATS.filter(function(x) { return x !== "الكل"; }).map(function(c) {
                            return '<option value="' + c + '"' + (f.cat === c ? ' selected' : '') + '>' + CI[c] + ' ' + c + '</option>';
                        }).join('') +
                    '</select>' +
    ...
    var catTables = CATS.filter(function(x) { return x !== "الكل"; }).map(function(cat) {
        var items = sortItems(S.stock.filter(function(s) {
            return s.cat === cat && (!S.ssrch || nAr(s.n).includes(nAr(S.ssrch)));
        }));
        if (items.length === 0) return '';

        var isOpen   = !!S.openCats[cat] || !!S.ssrch;
        var shopV    = items.reduce(function(a, s) { return a + s.q * s.c; }, 0);
        var whV      = items.reduce(function(a, s) { return a + (s.wq || 0) * s.c; }, 0);
        var profit   = items.reduce(function(a, s) { return a + s.q * (s.p - s.c); }, 0);
        var lowCnt   = items.filter(function(s) { return s.q <= (S.settings.lowStockThreshold || 5); }).length;
        ...
        return '<div class="stock-category" data-cat="' + cat + '">' +
            '<div class="category-header" onclick="S.openCats[\'' + cat + '\']=!S.openCats[\'' + cat + '\'];render()">' +
                ...
                '<div class="category-stats">' +
                    '<span class="stat">🏪 ' + fmt(shopV)  + '</span>' +
                    '<span class="stat">📦 ' + fmt(whV)    + '</span>' +
                    '<span class="stat">📈 ' + fmt(profit) + '</span>' +
                    (lowCnt > 0 ? '<span class="stat low">⚠️ ' + lowCnt + ' ناقص</span>' : '') +
                '</div>' +
                '<div class="category-toggle">' + (isOpen ? '▲' : '▼') + '</div>' +
            '</div>' +
            (isOpen ?
                '<div class="category-table-wrapper"><table class="stock-table">' +
                    '<thead><tr>' +
...
function bindStockEvents() {
    ...
    document.querySelectorAll('.dest-btn').forEach(function(btn) {
        btn.onclick = function() { S.form.dest = this.getAttribute('data-dest'); render(); };
    });
    document.querySelectorAll('.sort-btn').forEach(function(btn) {
        btn.onclick = function() { S.sortBy = this.getAttribute('data-sort'); render(); };
    });
    document.querySelectorAll('.view-btn').forEach(function(btn) {
        btn.onclick = function() { S.stockView = this.getAttribute('data-view'); render(); };
    });
    document.querySelectorAll('.add-to-cart-btn').forEach(function(btn) {
        btn.onclick = function() { addToCartFromStock(parseInt(this.getAttribute('data-id'))); };
    });
    ...
    document.querySelectorAll('.stock-qty, .stock-price').forEach(function(inp) {
        inp.onchange = function() {
            ...
            if (item) { item[field] = val; save(); render(); }
        };
    });
}
