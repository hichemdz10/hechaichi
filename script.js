...
var pieChart = null; // reference for chart.js instance
...
function renderHeader() {
    return '<div class="header">'+
        ...
        '<button style="background:#f1f5f9;border:1px solid #e2e8f0;border-radius:12px;font-size:19px;padding:9px 13px;color:#334155" onclick="S.hideNums=!S.hideNums;render()">'+(S.hideNums?"👁":"🙈")+'</button>'+
        '<button onclick="toggleCart()" style="background:#f1f5f9;border:1px solid #e2e8f0;border-radius:12px;font-size:16px;padding:9px 13px;color:#334155;display:flex;align-items:center;gap:6px">'+
...
// ===== HOME (new layout with pie chart and category cards at bottom) =====
function renderHome() {
    // Normal content (stats + buttons) appears always except when camera is active
    var stats = [
    ...
    var actionBar = '<div style="display:flex;gap:12px;margin-bottom:24px">'+
        ...
        '<button id="camBtn" style="background:linear-gradient(135deg,#0277bd,#01579b);color:#fff;border:none;border-radius:16px;padding:12px 20px;font-size:20px;cursor:pointer;box-shadow:0 4px 12px rgba(2,119,189,.2)">📷</button>'+
        '<button onclick="openManualCart()" style="background:linear-gradient(135deg,#2e7d32,#1b5e20);color:#fff;border:none;border-radius:16px;padding:12px 20px;font-size:20px;cursor:pointer;box-shadow:0 4px 12px rgba(46,125,50,.2)">➕</button>'+
    '</div>';

    // Pie chart section (only if camera not active)
    var chartSection = '';
    ...
    // New category cards (modern redesign) - placed at bottom
    ...
    var finalContent = '<div class="cart-overlay" id="cartOverlay" onclick="closeCart()"></div>'+
        '<div class="home-grid" style="display:grid;grid-template-columns:1fr 360px;gap:20px">'+
        '<div>'+
            (S.cameraActiveInHome ? splitView : normalContent)+
        '</div>'+
        '<div class="cart-sidebar" id="cartPanel" style="background:#fff;border-radius:24px;padding:16px;border:1px solid #eef2f6;display:flex;flex-direction:column;box-shadow:0 8px 24px rgba(0,0,0,.05);position:fixed;top:0;right:0;bottom:0;width:360px;z-index:1000;transform:translateX(100%);transition:transform .35s cubic-bezier(.4,0,.2,1)">'+
            '<button onclick="closeCart()" id="closeCartBtn" style="background:linear-gradient(135deg,#c62828,#e53935);color:#fff;border:none;border-radius:12px;padding:10px;font-size:15px;font-weight:700;font-family:Tajawal,Arial;margin-bottom:12px;cursor:pointer">✕ إغلاق</button>'+
...
function renderHomeOnly() {
    ...
    if (tc) {
        ...
        bindHomeEvents();
        drawPieChart(); // redraw chart after rendering
    }
}
    ...
    var catTables = CATS.filter(function(x){ return x!=="الكل"; }).map(function(cat){
        ...
        var tbody = items.map(function(s,idx){
            ...
            return '<tr style="background:'+rb+'" onmouseover="this.style.background=\'#f8fafc\'" onmouseout="this.style.background=\''+rb+'\'">'+
                (S.stockView!=="warehouse"?'<td style="padding:8px 6px;border-bottom:1px solid #f0f0f0;text-align:center"><input type="number" value="'+s.q+'" onchange="var it=S.stock.find(function(x){return x.id==='+s.id+'});if(it){it.q=+this.value;save();}" style="width:60px;padding:6px 3px;border-radius:8px;border:1px solid #e2e8f0;text-align:center;font-size:13px;color:'+qc+';font-weight:700;background:#fff">  </td>':'')+
