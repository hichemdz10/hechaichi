...
function startSplitCamera() {
    ...
    navigator.mediaDevices.getUserMedia({ 
        video: { 
            ...
            width: { ideal: 3840, exact: 1920 },
            height: { ideal: 2160, exact: 1080 }
        }, 
        audio: false 
    })
...
function printReceipt() {
    ...
    var html = '<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>فاتورة #'+invoiceNum+'</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Tajawal,Arial,sans-serif;font-size:13px;direction:rtl;width:80mm;margin:0 auto;padding:4mm 3mm;color:#000}@media print{body{width:80mm;margin:0;padding:3mm 2mm}@page{size:80mm auto;margin:0}}.center{text-align:center}.dash{border:none;border-top:1px dashed #999;margin:6px 0}.solid{border:none;border-top:2px solid #000;margin:6px 0}table{width:100%;border-collapse:collapse}</style></head><body>'+
        ...
        '   <table><thead>   <th style="text-align:right;font-size:11px;padding:4px 3px;border-bottom:1.5px solid #000">الصنف</th><th style="font-size:11px;padding:4px 3px;border-bottom:1.5px solid #000">الكمية×السعر</th><th style="text-align:left;font-size:11px;padding:4px 3px;border-bottom:1.5px solid #000">المجموع</th> </thead><tbody>'+rows+'</tbody> </table><hr class="dash">'+
...
function renderHeader() {
    return '<div class="header">'+
        ...
        '<div style="flex:1">'+
        '<div style="color:#0f172a;font-weight:900;font-size:22px">مكتبة حشايشي</div>'+
        '<div style="color:#475569;font-size:13px">📍 مركز صالح باي</div>'+
        '<div style="display:flex;gap:7px;margin-top:5px;flex-wrap:wrap">'+
        '<div style="background:#f8fafc;border-radius:9px;padding:4px 11px;border:1px solid #eef2f6"><span style="color:#1e293b;font-size:12px;font-weight:600" id="dateLbl">'+dateStr()+'</span></div>'+
        '<div style="background:#f8fafc;border-radius:9px;padding:4px 13px;border:1px solid #eef2f6"><span style="color:#1e293b;font-size:15px;font-weight:800;font-family:monospace" id="clockLbl">'+clockStr()+'</span></div>'+
        '</div>'+
        '</div>'+
        '<div style="display:flex;gap:12px;flex-wrap:wrap;align-items:center">'+
        '<div class="info-card"><div>💰 مبيعات اليوم</div><div>'+H(fmt(tST())+" دج")+'</div></div>'+
        '<div class="info-card"><div>📈 صافي اليوم</div><div style="color:'+(tNet()>=0?'#2e7d32':'#c62828')+'">'+H(fmt(tNet())+" دج")+'</div></div>'+
        '<button style="background:#f1f5f9;border:1px solid #e2e8f0;border-radius:40px;font-size:19px;padding:9px 13px;color:#334155" onclick="S.hideNums=!S.hideNums;render()">'+(S.hideNums?"👁":"🙈")+'</button>'+
        '<button onclick="toggleCart()" style="background:#f1f5f9;border:1px solid #e2e8f0;border-radius:40px;font-size:16px;padding:9px 13px;color:#334155;display:flex;align-items:center;gap:6px">'+
...
function drawPieChart() {
    ...
    if (!ctx) return;
    var todaySales = tSales();
    var categorySales = {};
    todaySales.forEach(function(sale) {
        sale.items.forEach(function(item) {
            var cat = item.cat || "أخرى";
            if (!categorySales[cat]) categorySales[cat] = 0;
            categorySales[cat] += item.sum;
        });
    });
    var labels = Object.keys(categorySales);
    var data = labels.map(function(l) { return categorySales[l]; });
    var colors = labels.map(function(l) { return CC[l] || "#9ca3af"; });
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: { callbacks: { label: function(tooltipItem) {
                    ...
                    return tooltipItem.label + ': ' + fmt(val) + ' دج';
                } } }
            }
        }
    });
...
function renderHome() {
    ...
    var statsHTML = '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:24px">'+
        stats.map(function(s){ return '<div style="background:'+s.g+';border-radius:20px;padding:16px 12px;text-align:center;box-shadow:0 2px 12px rgba(0,0,0,.05);transition:transform .2s" onmouseover="this.style.transform=\'translateY(-3px)\'" onmouseout="this.style.transform=\'none\'">'+
            '<div style="font-size:28px;margin-bottom:6px">'+s.ic+'</div>'+
            '<div style="color:rgba(255,255,255,.8);font-size:12px;font-weight:600;margin-bottom:4px">'+s.l+'</div>'+
            '<div style="color:#fff;font-weight:800;font-size:16px">'+s.v+'</div>'+
        '</div>'; }).join('')+'</div>';
    var actionBar = '<div style="display:flex;gap:12px;margin-bottom:24px">'+
        '<input id="hmSrch" value="" oninput="S.srch=this.value" placeholder="🔍 بحث سريع..." class="inp" style="flex:1;border-color:#e2e8f0">'+
    ...
    if (!S.cameraActiveInHome) {
        chartSection = '<div class="chart-container">'+
            '<div class="chart-title"><span>📊</span> أفضل المبيعات اليوم حسب الفئة</div>'+
            '<canvas id="dailySalesChart" style="max-height:260px; width:100%"></canvas>'+
        '</div>';
    }
    var catCards = '<div style="margin-top:16px">'+
        '<div style="font-size:18px;font-weight:800;color:#0f172a;margin-bottom:16px;display:flex;align-items:center;gap:8px">🗂️ تصفح حسب الفئة</div>'+
        '<div class="category-grid">'+
        CATS.filter(function(x){ return x!=="الكل"; }).map(function(cat){
            var count = S.stock.filter(function(s){ return s.cat===cat; }).length;
            var icon = CI[cat] || "📦";
            var bgGrad = "linear-gradient(135deg, "+CC[cat]+"22, #ffffff)";
            return '<div class="category-card" onclick="goToStockCat(\''+cat+'\')">'+
                '<div class="category-icon" style="background:'+bgGrad+'">'+icon+'</div>'+
    ...
    if (S.cameraActiveInHome) {
        splitView = '<div id="splitViewContainer" style="position:fixed;top:0;left:0;right:0;bottom:0;z-index:2000;background:#000;display:flex;flex-direction:column;">'+
            '<div id="splitCameraContainer" style="flex:4; background:#000; position:relative; min-height:0;">'+
                ...
                '<div style="position:absolute; bottom:16px; left:0; right:0; text-align:center; color:#fff; background:rgba(0,0,0,.5); padding:6px; font-size:13px">وجه الكاميرا نحو الباركود</div>'+
            '</div>'+
    ...
    var finalContent = '<div class="cart-overlay" id="cartOverlay" onclick="closeCart()"></div>'+
        ...
        '<div class="cart-sidebar" id="cartPanel">'+
            '<button onclick="closeCart()" id="closeCartBtn" style="background:linear-gradient(135deg,#c62828,#e53935);color:#fff;border:none;border-radius:12px;padding:10px;font-size:15px;font-weight:700;margin-bottom:12px;cursor:pointer">✕ إغلاق</button>'+
        ...
        '<script>'+
            ...
            'document.getElementById("closeSplitCamera").onclick = function() {'+
                ...
                'render();'+
            '};'+
            'setTimeout(function() { drawPieChart(); }, 200);'+
        '<\/script>';
    return finalContent;
}
...
//  بعد السطر الأخير مباشرة (بعد } الأخيرة)
// ═══════════════════════════════════════════════
...
function injectLiveBar() {
    ...
    var barsHTML = last7.map(function(d, idx) {
        ...
        return '<div style="display:flex;flex-direction:column;align-items:center;gap:4px;flex:1">' +
            '<span style="font-size:9px;color:' + (d.val > 0 ? '#4ade80' : '#4e6180') + ';font-weight:700">' +
                (d.val > 0 ? fmt(d.val) : '—') +
            '</span>' +
            '<div style="width:100%;background:rgba(255,255,255,0.06);border-radius:4px;height:48px;display:flex;align-items:flex-end;overflow:hidden">' +
                '<div style="width:100%;height:' + (pct || 0) + '%;background:linear-gradient(to top,' +
                    (isToday ? '#10d988,#4ade80' : '#4f8ef7,#7eb3ff') +
                ');border-radius:4px;min-height:' + (d.val > 0 ? '4' : '0') + 'px"></div>' +
            '</div>' +
    ...
    var summaryItems = [
        { l: 'فواتير', v: String(tSales().length), c: '#7eb3ff' },
        { l: 'بالسلة', v: String(S.cart.length), c: '#fbbf24' },
    ...
    bar.innerHTML =
        // عنوان البار
        '<div style="width:100%;display:flex;align-items:center;gap:10px;margin-bottom:4px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,0.06)">' +
            '<span style="font-size:11px;font-weight:700;color:#4ade80;background:rgba(16,217,136,0.12);border:1px solid rgba(16,217,136,0.25);padding:3px 10px;border-radius:99px">🟢 مباشر</span>' +
            '<span style="font-size:14px;font-weight:800;color:#e8edf5">لوحة الأداء اليومي</span>' +
        '</div>' +

        // العمود 1: مخطط 7 أيام
        '<div style="flex:1;min-width:200px">' +
            '<div style="font-size:10px;font-weight:700;color:#8fa3c0;margin-bottom:10px;display:flex;align-items:center;gap:5px">' +
                '📊 مبيعات آخر 7 أيام' +
            '</div>' +
            '<div style="display:flex;gap:4px;align-items:flex-end;height:72px">' + barsHTML + '</div>' +
        '</div>' +

        // فاصل
        '<div style="width:1px;background:rgba(255,255,255,0.07);align-self:stretch;flex-shrink:0"></div>' +

        // العمود 2: أفضل منتجات
        ...
        '<div style="flex:1;min-width:155px">' +
            ...
            '<div style="display:grid;grid-template-columns:1fr 1fr;gap:7px">' +
                summaryItems.map(function(x) {
                    return '<div style="background:rgba(255,255,255,0.04);border-radius:9px;padding:8px 9px;border:1px solid rgba(255,255,255,0.06)">' +
                        '<div style="font-size:9px;color:#4e6180;font-weight:600;margin-bottom:2px">' + x.l + '</div>' +
   ...
   // ═══════════════════════════════════════════
// الإضافات النهائية — يحل محل كل شيء بعد
// السطر الأخير من الكود الأصلي
// ═══════════════════════════════════════════
    ...
    function renderPanel() {
        panel.innerHTML =
            '<div class="lsp-header" id="lspToggle">' +
                '<div class="lsp-title">⚠️ منتجات تحتاج تموين' +
                    '<span class="lsp-badge">' + low.length + ' صنف</span>' +
                '</div>' +
                '<span style="font-size:18px;color:var(--text-muted);display:inline-block;transform:' + (isOpen ? 'rotate(180deg)' : 'rotate(0)') + '">▼</span>' +
            '</div>' +
            (isOpen ?
                '<div>' +
                low.map(function(s) {
                    ...
                    return '<div class="lsp-item">' +
                        ...
                        '<button onclick="goToStockCat(\'' + s.cat + '\')" style="background:linear-gradient(135deg,' + col + ',' + col + 'cc);color:#fff;border:none;border-radius:8px;padding:5px 10px;cursor:pointer;font-size:11px;font-weight:700;font-family:Tajawal,Arial">تموين</button>' +
                    '</div>';
                }).join('') +
                '</div>'
            : '');
...
function injectLiveBar() {
    ...
    var barsHTML = last7.map(function(d, idx) {
        ...
        var isToday = (idx === 6);
        return '<div style="display:flex;flex-direction:column;align-items:center;gap:4px;flex:1">' +
            '<span style="font-size:9px;color:' + (isToday && d.val > 0 ? '#059669' : (d.val > 0 ? 'var(--accent-blue)' : 'var(--text-muted)')) + ';font-weight:700">' +
                (d.val > 0 ? fmt(d.val) : '—') + '</span>' +
...
window.addEventListener('load', function() { setTimeout(refreshExtras, 600); });
document.addEventListener('click', function() { setTimeout(refreshExtras, 400); });
